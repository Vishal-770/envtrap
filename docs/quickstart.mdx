...
- Starts an in-memory MITM TLS proxy and routes all outbound HTTP/HTTPS through it
...
```
 🌐 SECRET LEAK DETECTED   2026-06-13T12:00:00.000Z
────────────────────────────────────────────────────────────
...
|---|---|---|
| `network` | Leak logged, request forwarded | Leak logged, connection terminated (`403`/socket destroy) |
