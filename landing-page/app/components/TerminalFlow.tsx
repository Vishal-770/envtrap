...
export default function TerminalFlow() {
  ...
  return (
    <div className="w-full flex flex-col items-center py-4 select-none">
      ...
      {/* 1. Sleek Dark Terminal Mockup */}
      <div className="w-full max-w-[430px] bg-zinc-950 rounded-2xl border border-zinc-850 shadow-2xl overflow-hidden font-mono text-xs md:text-sm leading-relaxed text-zinc-300 transition-all hover:border-zinc-800">
        {/* Terminal Header */}
        ...
        <div className="p-5 md:p-6 space-y-4">
          ...
          {/* Blocked Badge */}
          <div className="flex items-center gap-1.5 bg-rose-950/30 border border-rose-900/40 rounded-md px-2.5 py-1 text-rose-400 w-fit">
      ...
      <div className="h-10 w-0.5 border-l-2 border-dashed border-zinc-800 relative">
        {pulseActive && (
      ...
      <div className="flex items-center gap-2.5 bg-zinc-900/60 border border-zinc-800 rounded-full px-5 py-2.5 shadow-sm text-xs font-bold text-zinc-250 z-10 hover:border-zinc-700 transition-colors duration-300">
        <Image
          src="/nodejs.webp"
          alt="Node.js logo"
          width={15}
        ...
        />
        <span className="text-zinc-300">Node.js Application</span>
      </div>
      ...
      <div className="flex items-center gap-2.5 bg-zinc-900 border border-zinc-800 rounded-full px-6 py-3 shadow-lg text-xs font-bold text-zinc-100 z-10 hover:border-zinc-750 transition-colors duration-300">
        ...
        <span className="text-zinc-100 font-extrabold tracking-wide">envtrap agent</span>
      </div>
      ...
      <div className="w-full relative mt-[-2px] flex flex-col items-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5 w-full max-w-3xl px-2">
          <FlowCard
            icon={<Terminal size={14} className="text-zinc-300" />}
            title="stdout / stderr"
            status="BLOCKED"
          />
          <FlowCard
            icon={<Lock size={14} className="text-zinc-300" />}
            title="HTTPS Traffic"
            status="BLOCKED"
          />
          <FlowCard
            icon={<HardDrive size={14} className="text-zinc-300" />}
            title="Subprocesses"
            status="BLOCKED"
          />
          <FlowCard
            icon={<Globe size={14} className="text-zinc-300" />}
            title="DNS Tunneling"
            status="BLOCKED"
          />
          <FlowCard
            icon={<Activity size={14} className="text-zinc-300" />}
...
function FlowCard({ icon, title, status, className = "" }: FlowCardProps) {
  return (
    <div
      className={`relative flex flex-col items-center bg-zinc-950/40 border border-zinc-900 rounded-xl p-4 shadow-inner hover:border-zinc-800 transition-all duration-300 group ${className}`}
    >
      {/* Top red dot blocking the connector line with ripple animation */}
      <div className="absolute -top-[9px] left-1/2 -translate-x-1/2 w-4.5 h-4.5 rounded-full bg-rose-950 border border-rose-900/60 flex items-center justify-center shadow-lg z-10 text-rose-400">
        <div className="collision-ripple" />
        <X size={8} className="stroke-[3.5] relative z-10" />
      </div>

      <div className="p-2 bg-zinc-900 rounded-lg mb-2 text-zinc-300 transition-transform duration-300 group-hover:scale-105">
        {icon}
      </div>

      <span className="text-[10px] font-bold text-zinc-300 text-center mb-2 leading-tight tracking-wide">
        {title}
      </span>

      <span className="px-2 py-0.5 rounded-full text-[8px] font-extrabold tracking-wider bg-rose-950/80 text-rose-400 border border-rose-900/50">
        {status}
      </span>
    </div>
  );
